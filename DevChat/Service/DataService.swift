...
class DataService {
    ...
    func getAllFeedMessages(handler: @escaping (_ messageArray: [Message]) -> ()) {
    ...
    func getAllMessagesFor(desiredGroup: Group, handler: @escaping (_ messagesArray: [Message]) -> ()) {
        ...
        REF_GROUPS.observeSingleEvent(of: .value) { (groupMessageSnapshot) in
            guard let groupMessageSnapShot = groupMessageSnapshot.children.allObjects as? [DataSnapshot] else { return }
            for groupMessage in groupMessageSnapShot {
                let content = groupMessage.childSnapshot(forPath: "message").value as! String
        ...
        REF_GROUPS.observeSingleEvent(of: .value) { (groupSnapshot) in
            ...
            for group in groupSnapshot {
                ...
                if membersArray.contains((Auth.auth().currentUser?.uid)!) {
                    ...
                    let group = Group(title: title, description: description, key: group.key, members: membersArray, membersCount: membersArray.count)
